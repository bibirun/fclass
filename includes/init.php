<?php
	...
	//定义目录常量
	//系统根目录
	define('HOME_ROOT',str_replace('\\','/',substr(__DIR__,0,strpos(__DIR__,'\includes'))));
	//前台公共目录
	///var_dump(HOME_ROOT);
	//exit;
	...
	//加载公共函数
	include_once HOME_INC . '/function.php';
	...
	//判断
	if($script_name == 'home_privilege.php' && ($act == 'login' || $act == 'signin' || $act == 'forgetpassword' || $act == 'get_new_pass') || $script_name == 'home.php' ){
		//不需要验证的
	}else{
		//需要验证
		if(!isset($_SESSION['user'])){
				...
				//没有session信息,跳转页面
				admin_redirect('home_privilege.php','请先登录',1);
			}
